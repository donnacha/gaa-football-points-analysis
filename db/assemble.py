...
# Match-archive competition keys -> province names, for deriving beaten
# provincial semi-finalists (the de-facto All-Ireland "prelim" tier).
ARCHIVE_PROV = {"connacht": "Connacht", "leinster": "Leinster",
                "munster": "Munster", "ulster": "Ulster"}
# Provincial semi-finalists are recorded as facts for the pure-knockout eras
# (1928-2000 plus the 2020-21 COVID knockout), where the provincial rounds ARE
# the lower All-Ireland rounds (no real All-Ireland quarter-final).
SF_LOSER_YEARS = set(range(1928, 2001)) | {2020, 2021}


def provincial_sf_losers(year, pf):
    """Beaten provincial semi-finalists per province, from the match archive.
    A SF loser = a team that contested a provincial semi-final but was not one
    of the two provincial finalists (robust to drawn-and-replayed semis).
    Returns {province: [losers]}; provinces with no recorded SF are absent."""
    path = os.path.join(HERE, "matches", "football", "senior", f"{year}.json")
    if not os.path.exists(path):
        return {}
    with open(path, encoding="utf-8") as f:
        matches = json.load(f)
    played = {}  # province -> set of teams that played a semi-final
    for m in matches:
        prov = ARCHIVE_PROV.get(m.get("competition"))
        if prov and "semi" in m.get("stage", "").lower():
            played.setdefault(prov, set()).update([m["team1"], m["team2"]])
    out = {}
    for prov, teams in played.items():
        fin = pf.get(prov, {})
        finalists = {fin.get("winner"), fin.get("loser")} - {None, ""}
        losers = sorted(teams - finalists)
        if losers:
            out[prov] = losers
    return out
...
def main():
    ...
    for year in range(1887, 1990):
        ...
        # de-facto All-Ireland "prelim" tier: beaten provincial semi-finalists
        if year in SF_LOSER_YEARS:
            for p, losers in provincial_sf_losers(year, pf).items():
                if p in pf:
                    pf[p]["semi_final_losers"] = losers
    ...
    out = {
        "_README": (
            ...
            "Each year lists the provincial finals (winner/beaten finalist, plus "
            "beaten provincial semi-finalists in semi_final_losers for the "
            "no-quarter-final knockout eras 1928-2000 and 2020-21, "
            "derived from the match archive) and the "
